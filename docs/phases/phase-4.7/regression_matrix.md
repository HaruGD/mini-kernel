...
| P47-R05 | 4.7C | Sleepable, atomic, IRQ, and emergency contexts permit only legal allocation/blocking operations. | Reserved: `make test-driver-context` | IRQ allocation uses only the bounded reserve; sleep/block from atomic/IRQ and all ordinary runtime calls from NMI/DF are rejected and counted. | Planned |
