...
| P47-R06 | 4.7D | MMIO authority is limited to one live bound BAR mapping and checked offset. | `make test-driver-mmio` | Wrong binding/owner/device/generation, I/O BAR, zero/overflow range, misalignment, out-of-range width, incompatible cache policy, and use-after-unmap fail without access. | Complete |
| P47-R07 | 4.7D | PCI MMIO VA mappings unmap and reuse safely. | `make test-pci-mmio-va` | Repeated subrange/full-BAR map/unmap, compatible sharing, reference release, coalescing, exhaustion, and fault rollback return VA/page/map counts to baseline. | Complete |
| P47-R08 | 4.7E | Coherent DMA returns distinct CPU and device addresses obeying owner, mask, alignment, boundary, zeroing, budget, and teardown rules. | `make test-dma-coherent`; `make test-uefi-smoke` | 32/64-bit masks, impossible constraints, stale/wrong-device free, partial failures, bus-master ordering, and a QEMU EDU DMA round trip pass with zero transient-buffer drift. | Complete |
