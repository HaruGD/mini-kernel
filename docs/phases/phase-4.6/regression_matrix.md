...
| P46-R07 | 4.6E | Remote wake and reschedule IPIs publish work before notification and cannot lose or duplicate a wake. | `make test-smp-ipi` (partial) | Idle/busy remote CPUs receive bounded reschedule requests; coalesced/duplicate IPIs are harmless; semaphore, condition, IPC, input, timer, and join wakeups complete once. | In progress |
| P46-R08 | 4.6E | Affinity and distribution select only online eligible CPUs and cannot strand runnable work. | `make test-smp-affinity` (partial) | Invalid/empty/offline masks fail; pinned threads remain eligible only on allowed CPUs; unpinned workload makes measured progress on multiple CPUs. | In progress |
