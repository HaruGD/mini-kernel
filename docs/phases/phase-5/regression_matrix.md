...
| P5S-R03 | 5S | Common syscall validation enforces user-memory, output, permission, handle, ownership, and explicit result semantics. | `make test-syscall-validation`; `make test-syscall-faults` | Boundary/cross-page buffers, racing unmap, wrong permissions/rights/types/generations, unknown calls/flags/versions, and injected copy/allocation failures return only cataloged errors with exact cleanup. | In progress: 5S-C user-memory target implemented; permission/fault matrix remains |
