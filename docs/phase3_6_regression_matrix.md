...
| R06 | Boot `.drv` handoff is bounded and verified | `tools/driver_build_integration_test.py`, `make test-uefi-smoke` | UEFI enforces count/size/format/signature bounds; BootInfo v3 reports a bounded module list and the kernel accepts only policy-selected names. |
