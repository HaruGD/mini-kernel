...
int main(){
 ...
 DriverDmaStats ds;driver_dma_get_stats(&ds);check(ds.streaming_mappings==0&&ds.pinned_sources==0&&ds.streaming_maps==3&&ds.streaming_unmaps==3);check(ds.sync_rejections>=3&&ds.segment_overflow==1);DriverAllocationStats as;driver_allocation_get_stats(&as);check(as.active==0&&as.pinned_ranges==0&&as.pinned_free_rejections==1);return failures?1:0;
}
'''
STUBS=r'''
#include <stdint.h>
