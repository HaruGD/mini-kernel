...
SERIAL = ROOT / "logs" / "serial_service_registry_smoke.log"
QEMU_LOG = ROOT / "logs" / "qemu_service_registry_smoke.log"
MONITOR_OUTPUT = Path("/tmp/os64_service_registry_smoke_monitor.txt")
...
def run() -> int:
    ...
    run_id = os.getpid()
    esp = Path(f"/tmp/os64_service_registry_{run_id}_esp.img")
    vars_image = Path(f"/tmp/os64_service_registry_{run_id}_vars.fd")
    ...
    try:
        ...
        send_command(process, "run usvc_c.elf")
        wait_for_serial("[usvc] ready name=demo pid=", 20)
        wait_for_serial("[usvc] exiting without explicit unregister", 20)
        wait_for_serial("state=returned term=exit code=0x00000000", 20)
        send_command(process, "services")
        wait_for_serial("=== SERVICES ===", 10)
        wait_for_serial("count=0x00000000", 10)
        wait_for_serial("(no services)", 10)
    finally:
    ...
    required = [
        "[usvc] ready name=demo pid=",
        "[usvc] exiting without explicit unregister",
        "=== SERVICES ===",
        "count=0x00000000",
        "(no services)",
    ]
    ...
    if missing:
        print("service registry smoke missing:", file=sys.stderr)
        for item in missing:
            print(item, file=sys.stderr)
        return 1

    print("service registry smoke OK")
    return 0
