...
test-desktop-layers:
	python3 ./tools/desktop_layers_test.py

test-desktop-session: uefi
	python3 ./tools/desktop_session_smoke.py
...
test-image-codecs:
	python3 ./tools/image_codecs_test.py

test-ui-rendering:
	python3 ./tools/ui_rendering_test.py

test-widget-sdk: uefi
	python3 ./tools/image_ui_qemu_smoke.py

test-responsive-window:
	python3 ./tools/window_sdk_test.py

test-phase5c: test-image-codecs test-ui-rendering test-responsive-window test-widget-sdk

test-terminal-model:
	python3 ./tools/terminal_model_test.py

test-gui-terminal: test-terminal-model uefi
	python3 ./tools/gui_terminal_smoke.py

test-phase5d: test-gui-terminal
...
$(PHASE5_ASSETS) &: ./tools/build_image_fixtures.py ./tools/png_to_osimg.py
	python3 ./tools/build_image_fixtures.py --output-dir $(PHASE5_ASSET_DIR)

./bin/os64.bin: ./bin/kernel64.bin $(USER_BINS) $(USER_ELFS) $(ROOT_DRIVER_PACKAGES) $(PHASE5_ASSETS) ./tools/build_fat32_root_image.py
